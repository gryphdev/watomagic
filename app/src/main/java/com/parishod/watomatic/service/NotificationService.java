...
public class NotificationService extends NotificationListenerService {
    ...
    private void sendReply(StatusBarNotification sbn) {
        ...
        NotificationData notificationData = new NotificationData(sbn);
        ...
        provider.generateReply(this, incomingMessage, notificationData,
            new ReplyProvider.ReplyCallback() {
                ...
                public void onFailure(String error) {
                    // Handle special action codes from BotJS (DISMISS, KEEP, SNOOZE)
                    if ("DISMISS".equals(error)) {
                        // Bot decided to dismiss - don't send reply
                        Log.d(TAG, "Bot decided to dismiss notification");
                        return;
                    } else if ("KEEP".equals(error) || "SNOOZE".equals(error)) {
                        // Bot decided to keep or snooze - use fallback
                        Log.d(TAG, "Bot decided to " + error + " - using fallback reply");
                    } else {
                        // Regular error - log and use fallback
