...
public class NotificationService extends NotificationListenerService {
    ...
    private void sendReply(StatusBarNotification sbn) {
        ...
        String fallbackReply = customRepliesData.getTextToSendOrElse();
        ...
        // Crear NotificationData
        String title = NotificationUtils.getTitle(sbn);
        String body = sbn.getNotification().extras.getString(android.app.Notification.EXTRA_TEXT);
        boolean isGroup = sbn.getNotification().extras.getBoolean("android.isGroupConversation", false);
        List<String> actions = new ArrayList<>(); // TODO: Extraer acciones de la notificación si es necesario
        ...
        // Obtener provider usando factory
        ...
        provider.generateReply(this, incomingMessage, notificationData,
            new ReplyProvider.ReplyCallback() {
                ...
                public void onFailure(String error) {
                    // Manejar acciones especiales (DISMISS, KEEP, SNOOZE) o usar fallback
                    if ("DISMISS".equals(error)) {
                        // No responder, solo descartar
                        cancelNotification(sbn.getKey());
                        Log.d(TAG, "Bot requested DISMISS action");
                    } else if ("KEEP".equals(error) || "SNOOZE".equals(error)) {
                        // Usar respuesta estática como fallback
                        Log.d(TAG, "Bot requested " + error + " action, using fallback");
                        sendActualReply(sbn, notificationWear, fallbackReply);
                    } else {
                        // Error real, usar fallback
