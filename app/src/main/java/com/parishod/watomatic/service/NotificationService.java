...
public class NotificationService extends NotificationListenerService {
    ...
    private void sendReply(StatusBarNotification sbn) {
        ...
        NotificationData notificationData = NotificationData.from(sbn);

        ReplyProvider replyProvider = ReplyProviderFactory.getProvider(preferencesManager);

        try {
            replyProvider.generateReply(this, incomingMessage, notificationData, new ReplyProvider.ReplyCallback() {
                @Override
            ...
            Log.e(TAG, "Unexpected error while generating reply", e);
            sendActualReply(sbn, notificationWear, fallbackReplyText);
        }
    }
    ...
    private String extractIncomingMessage(StatusBarNotification sbn) {
        CharSequence incomingMessageChars = sbn.getNotification().extras.getCharSequence(Notification.EXTRA_TEXT);
        return incomingMessageChars != null ? incomingMessageChars.toString() : null;
    }
